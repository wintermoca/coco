...
BEGIN_MESSAGE_MAP(CpracticeDlg, CDialogEx)
	...
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER1, &CpracticeDlg::OnNMCustomdrawSlider1)
END_MESSAGE_MAP()
...
void CpracticeDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	if (nFlags & MK_LBUTTON) { //마우스 왼쪽 버튼을 누르면 아래 코드를 실행
		CClientDC dc(this); //this를 이용하여 이 윈도우를 사용한다고 지정
		CPen pen(PS_DOT, n, m_col); // 사용할 팬의 종류, 굵기, 색상을 설정
		dc.SelectObject(&pen); 
		dc.MoveTo(opnt);
		dc.LineTo(point); 
	}
...
void CpracticeDlg::OnBnClickedButton2()
{
	CColorDialog AA; 
	if (AA.DoModal() == IDOK) {
		m_col = AA.GetColor(); //공통 색 대화 상자를 표시하고 사용자가 색을 선택할 수 있도록 합니다.
	}
}


void CpracticeDlg::OnNMCustomdrawSlider1(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	
	*pResult = 0;
	n = m_slider.GetPos(); //슬라이더의 현재 위치를 n의 값으로 지정
}
